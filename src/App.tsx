  ...
  const handleLogin = async () => {
    try {
      ...
      if (err.code === "auth/popup-blocked") {
        setError(
          "Trình duyệt đã chặn cửa sổ đăng nhập. Vui lòng cho phép hiện popup hoặc mở ứng dụng trong tab mới.",
        );
      } else {
        setError(
          "Đã xảy ra lỗi khi đăng nhập. Vui lòng thử lại hoặc mở trong tab mới.",
        );
      }
    }
  };
      ...
      <div className="w-full max-w-md p-8 rounded-[2rem] bg-neutral-900 border border-neutral-800 text-center space-y-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-neutral-500 text-sm mt-2 font-medium">
            Hệ thống ghi điểm trực tuyến thời gian thực
          </p>
        </div>
        ...
        <div className="space-y-4 relative z-10">
          ...
          <div className="p-6 bg-neutral-800/30 rounded-2xl border border-neutral-800/50 text-left space-y-3">
            ...
            <p className="text-[11px] text-neutral-500 leading-relaxed">
              Nếu không thể đăng nhập, hãy nhấn vào biểu tượng{" "}
              <span className="text-white font-bold">"Mở trong tab mới"</span>{" "}
              hoặc <span className="text-white font-bold">"Chia sẻ"</span> ở góc
              màn hình để mở ứng dụng toàn màn hình.
            </p>
          </div>
        </div>
      </div>
...
function TournamentBracket({
  ...
  matches: Match[];
  category?: string;
  format?: "16 đội" | "12 đội (Double Chance 1.5)";
}) {
  ...
  const displayCategory =
    category === "" || !category
      ? availableCategories.includes("Đôi nam nữ")
        ? "Đôi nam nữ"
  ...
  const tournamentIds = [
    ...new Set(filteredMatches.map((m) => m.id.split("_")[0])),
  ];
  ...
  const inferredFormat =
    latestTournamentMatches.length >= 28
      ? "16 đội"
      : "12 đội (Double Chance 1.5)";
  const tournamentFormat = format || inferredFormat;

  const rounds =
    tournamentFormat === "12 đội (Double Chance 1.5)"
      ? [
      ...
      : [
          {
            id: "round1",
            name: "Vòng 1",
            matchIndices: [1, 2, 3, 4, 5, 6, 7, 8],
          },
          {
            id: "round2_thang",
            name: "Vòng 2 (Nhánh Thắng)",
            matchIndices: [9, 10, 11, 12],
          },
          {
            id: "round2_thua",
            name: "Vòng 2 (Nhánh Thua)",
            matchIndices: [13, 14, 15, 16],
          },
  ...
  const getMatchByTitleIdx = (idx: number) => {
    return latestTournamentMatches.find(
      (m) => m.bracketInfo?.matchIndex === idx,
    );
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-neutral-900/10 rounded-[2rem] border border-dashed border-neutral-800 mx-4">
        ...
        <p className="text-neutral-500 font-bold uppercase tracking-widest text-[10px]">
          Chưa có sơ đồ cho hạng mục này
        </p>
        {category && (
          <p className="text-[9px] text-neutral-700 mt-2 italic">
            Hạng mục: {category}
          </p>
        )}
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div
                        className={`flex justify-between items-center text-[10px] font-bold transition-colors ${winner === "A" ? "text-emerald-400" : "text-neutral-400"}`}
                      >
                        <span className="truncate max-w-[120px]">
                          {match.teamA}
                        </span>
                        <span className="font-mono bg-neutral-950 px-2 py-0.5 rounded-md border border-neutral-800">
                          {match.scoreA}
                        </span>
                      </div>
                      <div
                        className={`flex justify-between items-center text-[10px] font-bold transition-colors ${winner === "B" ? "text-emerald-400" : "text-neutral-400"}`}
                      >
                        <span className="truncate max-w-[120px]">
                          {match.teamB}
                        </span>
...
function AdminDashboard({ user }: { user: FirebaseUser }) {
  ...
  const [teams, setTeams] = useState<{
    "16 đội": string[];
    "12 đội": string[];
  }>({ "16 đội": Array(16).fill(""), "12 đội": Array(12).fill("") });
  const [tournamentFormat, setTournamentFormat] = useState<
    "16 đội" | "12 đội (Double Chance 1.5)"
  >("16 đội");
  const [category, setCategory] = useState<"Đôi nam nữ" | "Đôi nam">(
    "Đôi nam nữ",
  );
  const [defaultRefereeEmail, setDefaultRefereeEmail] = useState("");
  const [filterCategory, setFilterCategory] = useState<
    "Tất cả" | "Đôi nam nữ" | "Đôi nam"
  >("Tất cả");
  const [loading, setLoading] = useState(false);
  const [viewMode, setViewMode] = useState<"list" | "table">("table");
  const [activeTab, setActiveTab] = useState<
    "create" | "tournament" | "monitor" | "history" | "bracket" | "luckywheel"
  >("monitor");
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      setMatches(
        snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Match),
      );
    });
  }, []);
  ...
  const activeMatches = matches
    ...
    .sort((a, b) => {
      ...
      return (
        (a.bracketInfo?.matchIndex || 0) - (b.bracketInfo?.matchIndex || 0)
      );
    });
  const historyMatches = matches
    ...
    .sort(
      (a, b) =>
        (b.bracketInfo?.matchIndex || 0) - (a.bracketInfo?.matchIndex || 0),
    );

  const currentTeams =
    tournamentFormat === "16 đội" ? teams["16 đội"] : teams["12 đội"];
  const currentWheelNames =
    tournamentFormat === "16 đội" ? wheelNames["16 đội"] : wheelNames["12 đội"];

  const applyMasterSchedule = async () => {
    if (
      !window.confirm(
        "Áp dụng lịch thi đấu chuẩn cho TẤT CẢ các trận? (Sân và Giờ sẽ bị ghi đè)",
      )
    )
      return;
    setLoading(true);
    try {
      ...
      for (const match of matches) {
        ...
        // The match id format is "T12-category-123456_m1", so we extract prefix and suffix
        const prefix = match.id.startsWith("T16-")
          ? "T16"
          : match.id.startsWith("T12-")
            ? "T12"
            : null;
        if (!prefix) continue;
        const mIndexMatch = match.id.match(/(_m\d+)$/);
        if (!mIndexMatch) continue;
        const suffix = mIndexMatch[1];

        const entry = MASTER_SCHEDULE.find(
          (s) => s.prefix === prefix && s.suffix === suffix,
        );
        if (entry) {
          if (
            match.court !== entry.court ||
            match.scheduledTime !== entry.time
          ) {
      ...
      alert(`Đã cập nhật lịch thi đấu thành công cho ${updatedCount} trận!`);
    } catch (e) {
      console.error(e);
  ...
  const handleDeleteAll = async () => {
    if (
      window.confirm(
        "CẢNH BÁO: Bạn có chắc chắn muốn XÓA TẤT CẢ trận đấu không? Dữ liệu sẽ không thể khôi phục.",
      )
    ) {
      setLoading(true);
      try {
        ...
        console.error(err);
        alert(
          "Có lỗi xảy ra khi xóa trận đấu: " +
  ...
  const getScheduleInfo = (prefix: string, matchIdx: number) => {
    const entry = MASTER_SCHEDULE.find(
      (s) => s.prefix === prefix && s.suffix === `_m${matchIdx}`,
    );
    if (entry) {
      return { court: entry.court, scheduledTime: entry.time };
    }
    return { court: `Sân ${(matchIdx % 3) + 1}`, scheduledTime: null };
  };
  ...
  const generateTournament12 = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 6; i++) {
        const sched = getScheduleInfo("T12", i + 1);
        await createMatch({
          id: mId(i + 1),
          court: sched.court,
      ...
      for (let i = 0; i < 3; i++) {
        const sched = getScheduleInfo("T12", i + 7);
        await createMatch({
          id: mId(i + 7),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 4; i++) {
        const sched = getScheduleInfo("T12", i + 10);
        await createMatch({
          id: mId(i + 10),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 2; i++) {
        const sched = getScheduleInfo("T12", i + 14);
        await createMatch({
          id: mId(i + 14),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      const schedG3 = getScheduleInfo("T12", 16);
      await createMatch({
        id: mId(16),
        court: schedG3.court,
        scheduledTime: schedG3.scheduledTime,
        category: category,
        teamA: "Thua M14",
        teamB: "Thua M15",
        status: "upcoming",
      ...
      // Chung kết: 1 match
      const schedCK = getScheduleInfo("T12", 17);
      await createMatch({
        id: mId(17),
        court: schedCK.court,
        scheduledTime: schedCK.scheduledTime,
        category: category,
        teamA: "Thắng M14",
        teamB: "Thắng M15",
        status: "upcoming",
  ...
  const getMatchAbbreviation = (match: Match) => {
  ...
  const generateTournament = async () => {
    ...
    try {
      ...
      // Vòng 1: Matches 1-8
      for (let i = 0; i < 8; i++) {
        const sched = getScheduleInfo("T16", i + 1);
        await createMatch({
          id: mId(i + 1),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 4; i++) {
        const sched = getScheduleInfo("T16", i + 9);
        await createMatch({
          id: mId(i + 9),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 4; i++) {
        const sched = getScheduleInfo("T16", i + 13);
        await createMatch({
          id: mId(i + 13),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 4; i++) {
        const sched = getScheduleInfo("T16", i + 17);
        await createMatch({
          id: mId(i + 17),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 4; i++) {
        const sched = getScheduleInfo("T16", i + 21);
        await createMatch({
          id: mId(i + 21),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      for (let i = 0; i < 2; i++) {
        const sched = getScheduleInfo("T16", i + 25);
        await createMatch({
          id: mId(i + 25),
          court: sched.court,
          scheduledTime: sched.scheduledTime,
      ...
      const schedG3 = getScheduleInfo("T16", 27);
      await createMatch({
        id: mId(27),
        court: schedG3.court,
        scheduledTime: schedG3.scheduledTime,
      ...
      const schedCK = getScheduleInfo("T16", 28);
      await createMatch({
        id: mId(28),
        court: schedCK.court,
        scheduledTime: schedCK.scheduledTime,
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    ...
                    <p className="text-neutral-500 text-xs font-bold uppercase tracking-widest mt-1">
                      {tournamentFormat === "16 đội"
                        ? "Hệ thống Double Elimination tự động"
                        : "Thể thức Double Chance 1.5"}
                    </p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {currentTeams
                    .slice(0, tournamentFormat === "16 đội" ? 16 : 12)
                    .map((team, idx) => (
                      <div key={idx} className="relative">
                        <span className="absolute -top-2 -left-2 w-6 h-6 bg-neutral-800 border border-neutral-700 rounded-lg flex items-center justify-center text-[10px] font-black text-emerald-500 z-10 shadow-lg">
                          {idx + 1}
                        </span>
                        <input
        ...
        {activeTab === "monitor" && (
          ...
          >
            <div className="flex justify-between items-center mb-6 px-2">
              ...
              <div className="flex gap-2">
                <span className="text-[10px] font-black bg-emerald-500/10 text-emerald-500 px-3 py-1 rounded-full flex items-center">
                  {activeMatches.length} Trận
                </span>
                <button
                  onClick={applyMasterSchedule}
                  disabled={loading || matches.length === 0}
                  className="text-[10px] font-black bg-sky-500/10 text-sky-500 px-3 py-1 rounded-full hover:bg-sky-500/20 transition-all disabled:opacity-50 uppercase tracking-widest"
                >
                  Áp dụng lịch
                </button>
                <button
            ...
            {viewMode === "list" && (
              <div className="grid gap-4">
                {activeMatches.map((match) => (
                  <div
                    key={match.id}
                    className="bg-neutral-900/50 border border-neutral-800 p-5 rounded-[2rem] flex items-center justify-between group hover:border-neutral-700 transition-all shadow-lg active:scale-[0.99] cursor-default"
                  >
                    <div className="space-y-1">
                      ...
                      <h3 className="font-black text-white text-xl tracking-tight uppercase italic">
                        {match.teamA}{" "}
                        <span className="text-neutral-700 not-italic mx-1">
                          vs
                        </span>{" "}
                        {match.teamB}
                      </h3>
                      <div className="flex gap-2 items-center">
                        ...
                        {match.status === "upcoming" && (
                          <span className="text-[9px] bg-neutral-800 text-neutral-400 px-3 py-1 rounded-full font-black uppercase tracking-widest">
                            Chờ thi đấu
                          </span>
                        )}
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-right">
                        ...
                        <p className="text-[10px] text-neutral-600 uppercase font-black tracking-[0.2em]">
                          Trận đấu
                        </p>
                      </div>
                      <button
            ...
            {viewMode === "table" && (
              <div className="bg-neutral-900/50 border border-neutral-800 text-neutral-300 p-4 rounded-3xl overflow-x-auto mt-4 mx-2 shadow-xl shadow-black/20">
                ...
                <div className="min-w-[600px]">
                  <table className="w-full text-center border-collapse border border-neutral-800">
                    <thead className="bg-neutral-800 text-emerald-400">
                      <tr>
                        <th className="p-2 border border-neutral-700 uppercase text-[10px] tracking-widest w-10">
                          STT
                        </th>
                        <th className="p-2 border border-neutral-700 uppercase text-[10px] tracking-widest w-1/4">
                          Sân 1
                        </th>
                        <th className="p-2 border border-neutral-700 uppercase text-[10px] tracking-widest w-1/4">
                          Sân 2
                        ...
                        <th className="p-2 border border-neutral-700 uppercase text-[10px] tracking-widest w-20">
                          Thời gian
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {Array.from(
                        ...
                        .sort()
                        .map((t, idx) => {
                          const mS1 = activeMatches.find(
                            (m) => m.scheduledTime === t && m.court === "Sân 1",
                          );
                          const mS2 = activeMatches.find(
                            (m) => m.scheduledTime === t && m.court === "Sân 2",
                          );
                          const mS3 = activeMatches.find(
                            (m) => m.scheduledTime === t && m.court === "Sân 3",
                          );

                          const renderCell = (m?: Match) => {
                            if (!m)
                              return <span className="text-neutral-600">-</span>;
                            return (
                              <div className="flex flex-col items-center justify-center p-1.5">
                                <span className="font-black text-sm text-emerald-400 leading-none mb-1">
                                  {getMatchAbbreviation(m)}
                                </span>
                                <span className="text-[10px] font-medium text-neutral-300 max-w-[140px] truncate leading-tight w-full opacity-90">
                                  {m.teamA}
                                </span>
                                <span className="text-[10px] font-medium text-neutral-300 max-w-[140px] truncate leading-tight w-full opacity-90 mt-0.5">
                                  {m.teamB}
                                </span>
                                {m.status === "live" && (
                                  <span className="text-[8px] bg-red-500/20 text-red-500 border border-red-500/50 px-2 py-0.5 rounded uppercase font-black tracking-widest mt-1 animate-pulse">
                          ...
                          return (
                            <tr
                              key={t}
                              className="even:bg-neutral-800/30 odd:bg-transparent transition-colors hover:bg-neutral-800/80"
                            >
                              <td className="p-2 border border-neutral-800 font-bold text-neutral-400">
                                {idx + 1}
                              </td>
                              <td className="p-0 align-middle border border-neutral-800">
                                {renderCell(mS1)}
                              </td>
                              <td className="p-0 align-middle border border-neutral-800">
                                {renderCell(mS2)}
                              ...
                              <td className="p-2 border border-neutral-800 font-bold text-sm text-emerald-400">
                                {t}
                              </td>
                            </tr>
                          );
                  ...
                  {Array.from(
                    new Set(
                      activeMatches.map((m) => m.scheduledTime).filter(Boolean),
                    ),
                  ).length === 0 && (
                    <div className="text-center py-6 text-neutral-500 border border-neutral-800 border-t-0 font-medium text-sm">
                      Chưa có lịch có thời gian đấu. Vui lòng cập nhật thời
                      gian.
                    </div>
                  )}
                  ...
                  <div className="mt-8 mb-4">
                    <table className="w-full text-left border-collapse border border-neutral-800 max-w-lg">
                      <thead className="bg-neutral-800 text-emerald-400">
                        <tr>
                          <th className="p-2 border border-neutral-700 uppercase text-[10px] tracking-widest w-28 font-black text-center">
                            Ký hiệu
                          </th>
                          <th className="p-2 border border-neutral-700 uppercase text-[10px] tracking-widest font-black px-4">
                            Diễn giải
                          </th>
                        </tr>
                      </thead>
                      <tbody className="text-neutral-300 text-xs font-medium">
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">VL</td>
                          <td className="p-1 border border-neutral-800 px-4">Vòng loại đôi nam</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">VLN</td>
                          <td className="p-1 border border-neutral-800 px-4">Vòng loại đôi nam nữ</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">PO</td>
                          <td className="p-1 border border-neutral-800 px-4">Playoff</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">PON</td>
                          <td className="p-1 border border-neutral-800 px-4">Playoff đôi nam nữ</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">SE</td>
                          <td className="p-1 border border-neutral-800 px-4">Tranh vé thắng vào tứ kết</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">W</td>
                          <td className="p-1 border border-neutral-800 px-4">Vé vớt - tranh vé vớt vào tứ kết</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">TK</td>
                          <td className="p-1 border border-neutral-800 px-4">Tứ kết</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">TKB</td>
                          <td className="p-1 border border-neutral-800 px-4">Tứ kết đôi nam nữ</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">BKN</td>
                          <td className="p-1 border border-neutral-800 px-4">Bán kết đôi nam nữ</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">GN3</td>
                          <td className="p-1 border border-neutral-800 px-4">Tranh giải 3 đôi nam nữ</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">CKN</td>
                          <td className="p-1 border border-neutral-800 px-4">Chung kết đôi nam nữ</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">BK</td>
                          <td className="p-1 border border-neutral-800 px-4">Bán kết</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">G3</td>
                          <td className="p-1 border border-neutral-800 px-4">Tranh giải 3</td>
                        </tr>
                        <tr className="even:bg-neutral-800/30">
                          <td className="p-1 border border-neutral-800 text-center font-bold text-emerald-400">CK</td>
                          <td className="p-1 border border-neutral-800 px-4">Chung kết</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
        ...
        {activeTab === "bracket" && (
          ...
          >
            <div className="flex justify-between items-center mb-6 px-2">
              <div className="space-y-1">
                <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-2">
                  <LayoutDashboard className="text-emerald-500" size={20} /> Sơ
                  đồ thi đấu
                </h2>
                <div className="flex gap-2 items-center mt-2 overflow-x-auto pb-1 scrollbar-hide">
                  {["Tất cả", "Đôi nam nữ", "Đôi nam"].map((cat) => (
          ...
          >
            ...
            <RandomWheel
              ...
              onNamesChange={(newNames) =>
                setWheelNames((prev) => ({
                  ...prev,
                  [tournamentFormat === "16 đội" ? "16 đội" : "12 đội"]:
                    newNames,
                }))
              }
              ...
              onTeamPicked={(winner) => {
                const limit = tournamentFormat === "16 đội" ? 16 : 12;
                const emptyIdx = currentTeams.findIndex(
                  (t, i) => i < limit && (!t || t.trim() === ""),
                );
                if (emptyIdx !== -1) {
                  const newTeams = [...currentTeams];
        ...
        {activeTab === "history" && (
          ...
          >
            <div className="flex justify-between items-center mb-6 px-2">
              ...
              <div className="flex gap-2 items-center">
                <span className="text-[10px] font-black bg-sky-500/10 text-sky-500 px-3 py-1 rounded-full">
                  {historyMatches.length} Kết quả
                </span>
                <button
                  onClick={handleDeleteAll}
                  disabled={loading || matches.length === 0}
            ...
            <div className="space-y-4 text-left">
              {historyMatches.map((match) => (
                <div
                  key={match.id}
                  className="bg-neutral-950 border border-neutral-900 p-6 rounded-[2rem] group hover:border-neutral-800 transition-all opacity-80 hover:opacity-100"
                >
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black bg-neutral-900 px-3 py-1 rounded-full text-neutral-500 uppercase tracking-widest">
                        {match.court}
                      </span>
                      <span className="text-[9px] font-black text-neutral-800 uppercase tracking-widest">
                        [{match.category}]
                      ...
                      {match.bracketInfo && (
                        <span className="text-[10px] font-black bg-neutral-900 px-3 py-1 rounded-full text-neutral-600 uppercase tracking-widest italic">
                          {match.bracketInfo.roundId}
                        </span>
                      )}
                      <span className="text-[10px] font-black bg-emerald-500/10 text-emerald-500 px-3 py-1 rounded-full uppercase tracking-widest italic">
                        Xong
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 items-center text-center gap-4">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[8px] font-black text-neutral-700 uppercase tracking-[0.3em] mt-1">
                        Kết quả t.đấu
                      </span>
                    </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-neutral-900/50 flex justify-center gap-4">
                    {match.setScores.map((score, idx) => (
                      <div
                        key={idx}
                        className="flex flex-col items-center px-4 py-2 bg-neutral-900/30 rounded-2xl border border-neutral-900"
                      >
                        <span className="text-[8px] font-black text-neutral-600 uppercase tracking-widest mb-1">
                          Hiệp {idx + 1}
                        </span>
              ...
              {historyMatches.length === 0 && (
                <div className="text-center py-20 bg-neutral-900/10 rounded-[2rem] border-2 border-dashed border-neutral-900">
                  <p className="text-neutral-700 font-bold uppercase tracking-widest text-[9px]">
...
function RefereeView({
  ...
  const handleCommitPlayoff = async () => {
    ...
    const categoryMatches = allMatches.filter(
      (m) =>
        (filterCategory === "Tất cả" || filterCategory === m.category) && m.id,
    );
    if (categoryMatches.length === 0) return;
    const currentTourneyId = categoryMatches[0].id.split("_")[0];
    const tourneyMatches = allMatches.filter((m) =>
      m.id.startsWith(currentTourneyId),
    );

    const playoffs = tourneyMatches.filter(
      (m) => m.bracketInfo?.roundId === "Vòng Vé Vớt",
    );
    if (playoffs.length !== 3) {
      alert(
        "Giải đấu này không phải định dạng 12 đội hoặc không tìm thấy các trận Vé Vớt.",
      );
      return;
    }
    if (!playoffs.every((m) => m.status === "finished")) {
      alert(
        "Phải chờ cả 3 trận Vé Vớt (Playoff) kết thúc mới có thể tính hiệu số.",
      );
      return;
    }
    ...
    if (
      !window.confirm(
        "Chốt kết quả Vé Vớt: Xét hiệu số 3 đội thắng, 2 đội điểm cao nhất sẽ vào Tứ Kết. Xác nhận?",
      )
    )
      return;
    ...
    try {
      const pWinners = playoffs.map((m) => {
        ...
        return {
          ...
          hieuSo,
          matchId: m.id,
          topScore: Math.max(m.scoreA, m.scoreB),
        };
      });
      ...
      // Find QF Matches
      const qfMatches = tourneyMatches.filter(
        (m) => m.bracketInfo?.roundId === "Tứ Kết",
      );
      const qf1 = qfMatches.find(
        (m) => m.teamA === "Vé Vớt 1" || m.teamB === "Vé Vớt 1",
      );
      const qf2 = qfMatches.find(
        (m) => m.teamA === "Vé Vớt 2" || m.teamB === "Vé Vớt 2",
      );
      ...
      alert(
        `Đã chốt xong! ${top1} (HS: ${pWinners[0].hieuSo}) và ${top2} (HS: ${pWinners[1].hieuSo}) vào Tứ Kết. Đội ${pWinners[2].team} bị loại.`,
      );
    } catch (err) {
      console.error(err);
  ...
  const handleEndMatch = async () => {
    ...
    if (
      !confirm(
        "Bạn chắc chắn muốn xác nhận kết thúc trận đấu? Kết quả sẽ được ghi vào lịch sử.",
      )
    )
      return;
    ...
    const loserName =
      currentMatch.scoreA > currentMatch.scoreB
        ? currentMatch.teamB
        : currentMatch.teamA;
    propagateWinner(currentMatch.id!, winnerName, loserName).catch(
      console.error,
    );
  ...
  const handlePoint = async (team: "A" | "B") => {
    ...
    const newAScore =
      team === "A" ? currentMatch.scoreA + 1 : currentMatch.scoreA;
    const newBScore =
      team === "B" ? currentMatch.scoreB + 1 : currentMatch.scoreB;
  ...
  const handleUndo = async () => {
    ...
    const newHistory = currentMatch.pointHistory
  ...
  const forceFinish = async (winner: "A" | "B") => {
    ...
    if (
      !confirm(
        `Bạn chắc chắn muốn kết thúc trận đấu và xử thắng cho ${winner === "A" ? currentMatch.teamA : currentMatch.teamB}?`,
      )
    )
      return;
      ...
      <div className="space-y-6">
        ...
        {showBracket ? (
          ...
          <div className="grid gap-4">
              ...
              .map((match) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-[9px] text-neutral-500 uppercase font-bold mt-2">
                      Trạng thái:{" "}
            ...
            {availableMatches.filter((m) => m.status !== "finished").length ===
              0 && (
              <div className="text-center py-20 text-neutral-500 bg-neutral-900/30 rounded-[2rem] border-2 border-dashed border-neutral-900 px-6">
                ...
                <p className="font-medium tracking-tight text-sm">
                  Bạn chưa được gán trận nào hoặc các trận đã kết thúc.
                </p>
                <p className="text-[10px] mt-2 opacity-50 uppercase tracking-widest">
                  Vui lòng đợi BTC cập nhật
                </p>
              </div>
            )}
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-neutral-900 rounded-[2.5rem] p-6 md:p-12 space-y-10 border-2 border-neutral-800 shadow-2xl overflow-hidden relative">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-neutral-700 uppercase tracking-widest italic">
                [{currentMatch.category}]
              </span>
            </div>
            <h3 className="text-xl font-bold text-white mt-1 uppercase tracking-tight">
              Trận đấu{" "}
              <span className="text-neutral-500 font-normal">/ 25 Pts</span>
            </h3>
          </div>
          <div className="flex gap-2">
            {currentMatch.setScores.map((s, i) => (
              <div
                key={i}
                className="px-3 py-1.5 rounded-xl bg-black text-[10px] font-mono border border-neutral-800 text-neutral-400"
              >
                {s.a}-{s.b}
              </div>
            ))}
        ...
        <div className="grid grid-cols-2 gap-4 md:gap-10">
          ...
          <div
            ...
            className={`p-8 md:p-14 rounded-[2rem] border-2 cursor-pointer flex flex-col items-center gap-6 transition-all active:scale-95 ${
              currentMatch.serving === "A"
                ? "bg-emerald-500/10 border-emerald-500 shadow-lg shadow-emerald-500/5"
                : "bg-neutral-800/30 border-neutral-800"
            }`}
          >
            {currentMatch.serving === "A" && (
              <span className="text-[9px] font-black text-black uppercase tracking-widest bg-emerald-500 px-3 py-1 rounded-full">
                GIAO CẦU
              </span>
            )}
            <div className="text-center">
              <p className="text-xs text-neutral-500 uppercase font-black tracking-widest mb-4 truncate max-w-[140px]">
                {currentMatch.teamA}
              </p>
            ...
            <div className="flex gap-2">
              {[0].map((i) => (
                <div
                  key={i}
                  className={`w-10 h-2 rounded-full transition-colors duration-500 ${i < currentMatch.setsA ? "bg-emerald-500 shadow-sm shadow-emerald-500/50" : "bg-neutral-900"}`}
                />
              ))}
            </div>
          </div>
          ...
          <div
            ...
            className={`p-8 md:p-14 rounded-[2rem] border-2 cursor-pointer flex flex-col items-center gap-6 transition-all active:scale-95 ${
              currentMatch.serving === "B"
                ? "bg-sky-500/10 border-sky-500 shadow-lg shadow-sky-500/5"
                : "bg-neutral-800/30 border-neutral-800"
            }`}
          >
            {currentMatch.serving === "B" && (
              <span className="text-[9px] font-black text-black uppercase tracking-widest bg-sky-500 px-3 py-1 rounded-full">
                GIAO CẦU
              </span>
            )}
            <div className="text-center">
            ...
            <div className="flex gap-2">
              {[0].map((i) => (
                <div
                  key={i}
                  className={`w-10 h-2 rounded-full transition-colors duration-500 ${i < currentMatch.setsB ? "bg-sky-500 shadow-sm shadow-sky-500/50" : "bg-neutral-900"}`}
                />
              ))}
            </div>
          </div>
        ...
        <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800">
          <h4 className="text-[10px] font-black uppercase text-neutral-500 tracking-widest mb-4">
            Lịch sử điểm
          </h4>
          <div className="space-y-2">
              ...
              .map((entry, idx) => {
                ...
                if (!match)
                  return (
                    <div
                      key={idx}
                      className="px-4 py-2 bg-neutral-800 rounded-lg text-sm text-white"
                    >
                      {entry}
                    </div>
                  );
                ...
                return (
                  <div
                    key={idx}
                    className="grid grid-cols-4 gap-2 px-4 py-3 bg-neutral-800 rounded-xl text-sm font-medium text-white items-center"
                  >
                    <span className="truncate">{teamName}</span>
                    <span>{score}</span>
                    <span className="col-span-2 text-right font-mono text-neutral-400">
                      {time}
                    </span>
                  </div>
                );
        ...
        {currentMatch.status === "finished" && (
          ...
          >
            ...
            <h2 className="text-5xl font-black text-white uppercase italic tracking-tighter leading-tight mb-2">
              {currentMatch.setsA > currentMatch.setsB
                ? currentMatch.teamA
                : currentMatch.teamB}{" "}
              <br /> CHIẾN THẮNG!
            </h2>
      ...
      <div className="p-6 bg-neutral-900/30 border border-neutral-800 rounded-[2rem] flex items-center gap-4 text-neutral-500">
        ...
        <p className="text-[10px] uppercase font-black tracking-widest leading-relaxed">
          Trọng tài: Điểm số bạn nhập sẽ được cập nhật TRỰC TIẾP lên bảng tổng
          hợp của Ban tổ chức.
        </p>
      </div>
    </div>
...
const MASTER_SCHEDULE = [
  { prefix: "T16", suffix: "_m1", time: "07:30", court: "Sân 1" },
  { prefix: "T16", suffix: "_m2", time: "07:30", court: "Sân 2" },
  { prefix: "T16", suffix: "_m3", time: "07:30", court: "Sân 3" },
  { prefix: "T16", suffix: "_m4", time: "07:45", court: "Sân 1" },
  { prefix: "T16", suffix: "_m5", time: "07:45", court: "Sân 2" },
  { prefix: "T16", suffix: "_m6", time: "07:45", court: "Sân 3" },
  { prefix: "T16", suffix: "_m7", time: "08:00", court: "Sân 1" },
  { prefix: "T16", suffix: "_m8", time: "08:00", court: "Sân 2" },
  { prefix: "T16", suffix: "_m13", time: "08:00", court: "Sân 3" }, // PO1
  { prefix: "T16", suffix: "_m14", time: "08:15", court: "Sân 1" }, // PO2
  { prefix: "T16", suffix: "_m15", time: "08:15", court: "Sân 2" }, // PO3
  { prefix: "T16", suffix: "_m16", time: "08:15", court: "Sân 3" }, // PO4
  { prefix: "T16", suffix: "_m9", time: "08:30", court: "Sân 1" },  // SE1
  { prefix: "T16", suffix: "_m10", time: "08:30", court: "Sân 2" }, // SE2
  { prefix: "T16", suffix: "_m11", time: "08:30", court: "Sân 3" }, // SE3
  { prefix: "T16", suffix: "_m12", time: "08:45", court: "Sân 1" }, // SE4
  { prefix: "T12", suffix: "_m1", time: "08:45", court: "Sân 2" },  // VLN1
  { prefix: "T12", suffix: "_m2", time: "08:45", court: "Sân 3" },  // VLN2
  { prefix: "T12", suffix: "_m3", time: "09:00", court: "Sân 1" },  // VLN3
  { prefix: "T12", suffix: "_m4", time: "09:00", court: "Sân 2" },  // VLN4
  { prefix: "T12", suffix: "_m5", time: "09:00", court: "Sân 3" },  // VLN5
  { prefix: "T12", suffix: "_m6", time: "09:15", court: "Sân 1" },  // VLN6
  { prefix: "T16", suffix: "_m17", time: "09:15", court: "Sân 2" }, // W1
  { prefix: "T16", suffix: "_m18", time: "09:15", court: "Sân 3" }, // W2
  { prefix: "T16", suffix: "_m19", time: "09:30", court: "Sân 1" }, // W3
  { prefix: "T16", suffix: "_m20", time: "09:30", court: "Sân 2" }, // W4
  { prefix: "T12", suffix: "_m7", time: "09:30", court: "Sân 3" },  // PON1
  { prefix: "T12", suffix: "_m8", time: "09:45", court: "Sân 1" },  // PON2
  { prefix: "T12", suffix: "_m9", time: "09:45", court: "Sân 2" },  // PON3
  { prefix: "T16", suffix: "_m21", time: "09:45", court: "Sân 3" }, // TK1
  { prefix: "T16", suffix: "_m22", time: "10:00", court: "Sân 1" }, // TK2
  { prefix: "T16", suffix: "_m23", time: "10:00", court: "Sân 2" }, // TK3
  { prefix: "T16", suffix: "_m24", time: "10:00", court: "Sân 3" }, // TK4
  { prefix: "T12", suffix: "_m10", time: "10:15", court: "Sân 1" }, // TKN1
  { prefix: "T12", suffix: "_m11", time: "10:15", court: "Sân 2" }, // TKN2
  { prefix: "T12", suffix: "_m12", time: "10:15", court: "Sân 3" }, // TKN3
  { prefix: "T12", suffix: "_m13", time: "10:30", court: "Sân 1" }, // TKN4
  { prefix: "T16", suffix: "_m25", time: "10:45", court: "Sân 1" }, // BK1
  { prefix: "T16", suffix: "_m26", time: "10:45", court: "Sân 2" }, // BK2
  { prefix: "T12", suffix: "_m14", time: "11:00", court: "Sân 1" }, // BKN1
  { prefix: "T12", suffix: "_m15", time: "11:00", court: "Sân 2" }, // BKN2
  { prefix: "T16", suffix: "_m27", time: "11:15", court: "Sân 2" }, // G3
  { prefix: "T12", suffix: "_m16", time: "11:30", court: "Sân 1" }, // GN3
  { prefix: "T16", suffix: "_m28", time: "11:30", court: "Sân 2" }, // CK
  { prefix: "T12", suffix: "_m17", time: "11:45", court: "Sân 2" }, // CKN
];
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {!isAdmin && user && (
                <p className="text-[7px] text-neutral-700 font-mono mt-0.5 select-all">
                  UID: {user.uid}
                </p>
              )}
            </div>
          ...
          <div className="flex items-center gap-3">
            {!isAdmin &&
              user?.email &&
              ["baopa0805@gmail.com", "tongducduy4@gmail.com"].includes(
                user.email,
              ) && (
                <button
                  onClick={async () => {
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {view === "admin" ? (
              ...
              <RefereeView isAdmin={isAdmin} user={user} />
            )}
          </motion.div>
        </AnimatePresence>
      </main>
      ...
      <footer className="max-w-4xl mx-auto p-12 text-center space-y-4 border-t border-neutral-900/50 mt-12">
        ...
        <div className="flex justify-center items-center gap-3">
          <div className="px-3 py-1 bg-emerald-500/5 rounded-full flex items-center gap-2 border border-emerald-500/10">
            ...
            <span className="text-[9px] font-black uppercase tracking-tight text-emerald-500/80">
              Hệ thống sẵn sàng
            </span>
          </div>
        </div>
